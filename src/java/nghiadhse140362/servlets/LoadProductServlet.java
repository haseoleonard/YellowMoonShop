    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            ...
            if(txtProductID!=null&&!txtProductID.trim().isEmpty()&&txtProductID.matches("\\d+")){
                ...
                boolean rs = productsDAO.loadSelectedProduct(productID, true);
                if(rs){
                    request.setAttribute("LOADED_PRODUCT", productsDAO.getSelectedProduct());
                }
            }
        } catch (NamingException | SQLException ex) {
            LOGGER.error(ex.getMessage());
        }finally{
            request.getRequestDispatcher(Constants.UPDATE_PRODUCT_PAGE).forward(request, response);
        }
    }
