...
//            log("CreateAccountFilter:doFilter()");
        ...
        try {
            ...
            //
            if(txtUsername==null||txtUsername.trim().isEmpty()||txtUsername.length()<6||txtUsername.length()>50){
                error.setUsernameLengthErr("Username Length must be between 6-50 chars");
                valid = false;
            }
            //
            if(txtName==null||txtName.trim().isEmpty()||txtName.length()>250){
                error.setNameLengthErr("Name must not be empty and under 250 chars");
                valid = false;
            }
            //
            if(txtAddress==null||txtAddress.trim().isEmpty()){
                error.setAddressLengthErr("Address Must not be Empty and under 400 chars");
                valid=false;
            }
            //
            if(txtPhone==null||txtPhone.trim().isEmpty()||!txtPhone.matches("\\d+")){
                error.setInvalidPhoneErr("Invalid Phone Number");
                valid=false;
            }
            //
            if(password==null||password.trim().isEmpty()||password.length()<6||password.length()>64){
                error.setPasswordLengthErr("Password must be between 6-64 Characters");
                valid=false;
